...
const FONT_IMPORT = "@import url('https://fonts.googleapis.com/css2?family=Playball&family=Fraunces:opsz,wght@9..144,400;9..144,600;9..144,700&family=Plus+Jakarta+Sans:wght@400;500;600;700;800&display=swap');";
const MODULE_META = {
...
const TODAY = /* @__PURE__ */ new Date("2026-07-31");
const daysUntil = (iso) => Math.ceil((new Date(iso).getTime() - TODAY.getTime()) / 864e5);
const seedDeadlines = () => [
  {
    id: 1,
    type: "Assignment",
...
function MiniCalendar({
  ...
  >
        ...
        {cells.map((day, i) => <div key={i} className="flex items-center justify-center py-0.5">
            {day && <div
  ...
  >
                {day}
                {dueDays.has(day) && !isToday(day) && <span
...
function StatCard({
  ...
  >
        ...
        {sub && <div className="text-[11px]" style={{ color: C.inkSoft }}>
            {sub}
          </div>}
      </div>
    </div>;
...
function StatusPill({
  ...
  const icon = status === "done" ? <CheckCircle2 size={14} /> : status === "progress" ? <Clock size={14} /> : <Circle size={14} />;
  const bg = status === "done" ? "#E4F5EB" : status === "progress" ? "#FBF0DA" : C.lilac50;
  return <button
    onClick={onClick}
...
function StudyDashboard() {
  ...
  const cycleTopic = (id) => setTopics(
    (ts) => ts.map(
      (t) => t.id === id ? { ...t, status: STATUS_CYCLE[t.status] } : t
    )
  );
  const cycleDeadline = (id) => setDeadlines(
    (ds) => ds.map(
      (d) => d.id === id ? { ...d, status: STATUS_CYCLE[d.status] } : d
    )
  );
  const stats = useMemo(() => {
    ...
    const nextDeadline = [...deadlines].filter((d) => d.status !== "done").sort((a, b) => new Date(a.due).getTime() - new Date(b.due).getTime())[0];
  ...
  const goals = [
    {
      ...
      pct: stats.pct,
      detail: "All 5 modules \xB7 due 30 Sep"
    },
  ...
  >
        ...
        {tab === "dashboard" && <div className="flex flex-col gap-5">
            {
    /* Hero row */
  }
            <div
  ...
  >
    ...
    style={{ fontFamily: "Fraunces, serif" }}
  >
                    {stats.nextDeadline ? `${daysUntil(stats.nextDeadline.due)}d` : "\u2014"}
                  </div>
                  <div className="text-[11px]" style={{ color: "#F6C6DE" }}>
  ...
  />
    ...
    sub={stats.nextDeadline ? `${daysUntil(stats.nextDeadline.due)} days left` : ""}
  />
            </div>
  ...
  />
    ...
    formatter={(_value, _name, item) => [
      `${item.payload?.done ?? 0} / ${item.payload?.total ?? 0} topics`,
      "Done"
    ]}
  />
                    <Bar dataKey="done" radius={[8, 8, 0, 0]}>
                      {stats.perModule.map((m, i) => <Cell key={i} fill={m.color} />)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              </SectionCard>
  ...
  >
    ...
    style={{
      width: `${g.pct}%`,
      background: g.pct >= 70 ? "#3E9A6D" : g.pct >= 35 ? C.violet500 : C.gold,
  ...
  >
    ...
    style={{
      background: daysUntil(d.due) < 0 ? "#F3E4E4" : C.lilac50,
  ...
  />
    ...
    formatter={(_v, _n, item) => [
      `${item.payload.total} topics`,
      "Total"
    ]}
  />
                    <Bar dataKey="total" radius={[0, 8, 8, 0]}>
                      {topicsByModule.map((m, i) => <Cell key={i} fill={m.color} />)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              </SectionCard>
  ...
  >
    ...
    style={{
      background: "linear-gradient(135deg, #EFD79B, #D9A441)"
    }}
  />
                </div>
                <div>
    ...
    style={{ fontFamily: "Fraunces, serif" }}
  >
                    {stats.nextDeadline ? daysUntil(stats.nextDeadline.due) : "\u2014"}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1.4fr_1fr] gap-5">
              <SectionCard title="Modules at a Glance" icon={BookOpen}>
                <div className="flex flex-col gap-3">
                  {stats.perModule.map((m) => <div key={m.full}>
  ...
  >
                  ...
                  <button
    onClick={() => {
      if (window.confirm("Reset all progress back to default?")) {
  ...
  >
        ...
        {tab === "tracker" && <div className="flex flex-col gap-3">
    ...
    />
                  ...
                  {open && <div className="px-4 pb-4 flex flex-col gap-4">
                      {chapters.map((ch) => <div key={ch}>
                          <div
    ...
    >
                          ...
                          <div className="flex flex-col gap-1.5">
                            {list.filter((t) => t.chapter === ch).map((t) => <div
      key={t.id}
      className="flex items-center justify-between gap-2 py-1.5 px-2 rounded-lg"
      style={{
        background: t.status === "done" ? "#F3FBF6" : "transparent"
      }}
    >
                                  <span
    ...
    />
        ...
        {tab === "deadlines" && <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {deadlines.map((d) => <div
    key={d.id}
  ...
  >
                    {daysUntil(d.due) < 0 ? "Past due" : `${daysUntil(d.due)} days left`}
                  </span>
                </div>
                <div>
                  <div
